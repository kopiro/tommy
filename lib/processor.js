...
async function processVideoThumbs(tommy, filepath) {
  ...
  const dst_path = filepath.replace(
    util.REGEX_EXT,
    config.suffix.replace('${i}', '%d')
  );
  console.debug(`Extracting thumbnails to <${dst_path}>`);

  let video_duration = await util.execPromise(
    `ffprobe -hide_banner -i "${filepath}" -show_format -v quiet | sed -n 's/duration=//p'`
  );
  ...
  for (let i = 1; i <= config.count; i++) {
    const url = `${filepath.replace(util.REGEX_EXT, '')}${config.suffix.replace(
      '${i}',
      i
    )}`;
...
async function processResize(tommy, filepath) {
  ...
  for (let px of config.dimensions) {
    if (px < largest_side) {
      ...
      const dst_path = filepath.replace(
        util.REGEX_EXT,
        config.suffix.replace('${i}', px).replace('${ext}', ext)
      );
  ...
  for (let img of resized_images) {
    await converter.webp(tommy, img);
  }
...
<html>
	<head>
		<link rel="apple-touch-icon" sizes="180x180" href="${path.basename(
      dst_paths.ios
    )}">
		<link rel="icon" type="image/png" sizes="32x32" href="${path.basename(
      dst_paths.favicon_32
    )}">
		<link rel="icon" type="image/png" sizes="16x16" href="${path.basename(
      dst_paths.favicon_16
    )}">` +
        (dst_paths.webmanifest
          ? `<link rel="manifest" href="${path.basename(
              dst_paths.webmanifest
            )}">`
          : ``) +
        (config.tileColor
...
module.exports = {
  ...
  videoThumbs: processVideoThumbs,
  resize: processResize,
  lazyLoadBlurried: processLazyLoadBlurriedImage,
