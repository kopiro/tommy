...
async function testFont(tommy, filepath) {
  ...
  console.debug(
    `Extracting HTML test for font-face declaration to <${html_file}>`
  );
...
async function testImage(tommy, filepath) {
  const format = util.getExtension(filepath);
  ...
  const html_string =
    ...
    `<h2>Conversions</h2>
		...
		<hr/>` +
    `<h2>Resizes</h2>` +
    (tommy.config["processor.resize"].dimensions || [])
      .map(px => {
        const url =
          file_name +
          tommy.config["processor.resize"].suffix
            .replace("${i}", px)
            .replace("${ext}", format);
    ...
    `<h2>Lazy Load Blurried</h2>
		<h3>${file_name}${tommy.config["processor.lazyLoadBlurried"].suffix}</h3>
		<img style="width: 100%" src="${file_name}${
      tommy.config["processor.lazyLoadBlurried"].suffix
    }" />
		<hr/>` +
    `</body>`;
...
async function testVideo(tommy, filepath) {
  ...
  const html_string =
    `<!doctype html>
		...
		<hr/>` +
    `<h2>Conversions</h2>
		<h3>${file_name}.h264.mp4</h3>
		<video src="${file_name}.h264.mp4" controls></video>
		<h3>${file_name}.hevc.mp4</h3>
		<video src="${file_name}.hevc.mp4" controls></video>
		<h3>${file_name}.av1.mp4</h3>
		<video src="${file_name}.av1.mp4" controls></video>
		<h3>${file_name}.webm</h3>
		<video src="${file_name}.webm" controls></video>
		<hr/>` +
    `<h2>Video Thumbs</h2>` +
    new Array(tommy.config["processor.videoThumbs"].count)
      .fill(0)
      .map((_, i) => i + 1)
      .map(i => {
        const url = `${file_name}${tommy.config[
    ...
    `<hr/>` +
    `<h2>Poster</h2>
		<h3>${file_name}${tommy.config["processor.poster"].suffix}</h3>
		<img src="${file_name}${tommy.config["processor.poster"].suffix}" />
		<hr/>` +
    `</body>`;
