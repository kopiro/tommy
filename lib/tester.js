...
async function testImage(tommy, filepath) {
	...
	const html_string = `<!doctype html>
		...
		`<h2>Resizes</h2>` +
		(tommy.config['processor.resize'].dimensions || []).map(px => {
			const url = file_name + tommy.config['processor.resize'].suffix.replace('${i}', px).replace('${ext}', format);
		...
		<img style="width: 100%" src="${file_name}${tommy.config['processor.lazyLoadBlurried'].suffix}" />
		<hr/>` +
...
async function testVideo(tommy, filepath) {
	...
	const html_string = `<!doctype html>
		...
		`<h2>Video Thumbs</h2>` +
		(new Array(tommy.config['processor.videoThumbs'].count).fill(0).map((_, i) => (i + 1))).map(i => {
			const url = `${file_name}${tommy.config['processor.videoThumbs'].suffix.replace('${i}', i)}`;
			return `<h3>${url}</h3><img src="${url}" />`;
		}).join('') +
		`<hr/>` +
